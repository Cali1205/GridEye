...
uint8_t grideye::init (uint8_t address_number)
{
    ...
    address = GRID_EYE_ADDRESS;//address_number == 0 ? 0b1101000 : 0b1101001;

	//pinMode(5, OUTPUT);
	//pinMode(4, INPUT);
    //Wire.begin();
    Wire.beginTransmission(GRID_EYE_ADDRESS);
    Wire.write(GE_FPSC_REG,1);
...
bool grideye::bAMG_PUB_I2C_Read (uint8_t ucRegAddr, uint8_t ucSize, uint8_t* ucDstAddr)
{
	...
	Wire.requestFrom(GRID_EYE_ADDRESS, ucSize);
...
------------------------------------------------------------------------------*/
void vAMG_PUB_TMP_ConvTemperature64( UCHAR* pucRegVal, float* pshVal )
{
	UCHAR ucCnt = 0;
